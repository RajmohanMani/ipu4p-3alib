...
*/
ia_err ia_ltm_set_tuning(ia_ltm *ia_ltm,
 ...
 * may or may not contain WB gains and CCM applied. Parameters needed by Optibright algorithm are marked with '(ob)' and parameters needed by CLAHE are
 * marked with '(cl)'.
 */
typedef struct ia_ltm_input_params
{
    ...
    ia_aiq_rgbs_grid *rgbs_grid_ptr;        /*!< Optional. (cl) RGBS statistics. LTM may use this small grid instead of given larger grids to reduce PnP (available in IPU4 and onwards). */
    ia_aiq_hdr_rgbs_grid *hdr_rgbs_grid_ptr;/*!< Optional. (cl) HDR RGBS statistics. LTM may use this small grid instead of given larger grids to reduce PnP (available in IPU4 and onwards). */
    ia_ltm_input_image *input_image_ptr;    /*!< Optional. (ob)(cl) Image data of any resolution based on IQ requirements for particular use case from which LTM calculates local tone maps (HW generated image available and onwards). */
