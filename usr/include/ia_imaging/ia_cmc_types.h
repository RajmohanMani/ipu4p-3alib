...
/*!
 * \brief Color format definitions.
 */
typedef enum
{
    cmc_media_format_srgb,
    cmc_media_format_bt601,
    cmc_media_format_bt709,
    cmc_media_format_bt2020,
    cmc_media_format_bt2100_rgb,
    cmc_media_format_bt2100_lms,
    cmc_media_format_dci_p3,
    cmc_media_format_adobergb,
} cmc_media_format;
...
typedef enum
{
    ...
    cmc_name_id_analog_gain_conversions,                 /*!< 34 */
} cmc_name_id;
...
typedef struct
{
    ...
    uint8_t  conversion_gain_ratio;     /*!< Conversion gain ratio for the sensor. */
} cmc_general_data_t;
...
} cmc_analog_gain_segment2_t;

typedef struct
{
    cmc_analog_gain_conversion_type_t conversion_type; /*!< Analog gain conversion type. */
    uint16_t num_segments;              /*!< Number of gain/code segments which describe the analog gain. */
    uint16_t num_pairs;                 /*!< Number of gain/code pairs which describe the analog gain. */
    union {
        cmc_analog_gain_segment2_t *ptr;/*!< Array of analog gain segments. Array length is defined by num_segments. */
        uint64_t dummy;                 /* Ensures that pointer is accessible in both 32 and 64 bit systems. */
    } segments;
    union {
...
} cmc_analog_gain_conversion2_t;

typedef struct
{
    ia_mkn_record_header header;                /*!< Record header with Format ID: UInt16 (See AIQB_DataID), Name ID: cmc_name_id_hdr_analog_gain_conversion (See cmc_name_id). */
    uint32_t num_exposures;                     /*!< Number of exposures for which analog gain conversion is available. */
    uint32_t padding;
    union
    {
        cmc_analog_gain_conversion2_t* ptr;  /*!< Array of gain conversions, for each exposure separately. */
        uint64_t dummy;                         /* Ensures that pointer is accessible in both 32 and 64 bit systems. */
    } conversions;
} cmc_analog_gain_conversions_t;
...
typedef struct
{
    ...
    int32_t gdc_mode;                                  /*!< GDC mode enum, 0=Grid based (old),  1=LUT based (new) */
...
typedef struct
{
    float hdr_exposure_time_ratio_min;  /*!< Minimum allowed ratio between exposure times. */
    float hdr_exposure_time_ratio_max;  /*!< Maximum allowed ratio between exposure times. */
    float hdr_analog_gain_ratio_min;    /*!< Minimum allowed ratio between analog gains. */
    float hdr_analog_gain_ratio_max;    /*!< Maximum allowed ratio between analog gains. */
    float hdr_digital_gain_ratio_min;   /*!< Minimum allowed ratio between digital gains. */
    float hdr_digital_gain_ratio_max;   /*!< Maximum allowed ratio between digital gains. */
...
typedef struct
{
    ...
    cmc_parsed_analog_gain_conversion_t cmc_parsed_analog_gain_conversion; /* DEPRECATED. Use cmc_analog_gain_conversion instead. */
    ...
    cmc_analog_gain_conversions_t *cmc_analog_gain_conversions;
