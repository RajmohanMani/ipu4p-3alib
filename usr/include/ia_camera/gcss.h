...
public:
     ...
     *  being moved to match the returned descendant. Returns NULL if no
     *  descendant is found. GCSS_KEY_NA can be used to iterate over all
     *  descendants.
...
public:
    ...
    * \ingroup gcss
    */
    NodeIterator(const IGraphConfig* node) : mCurrentIndex(0), mTarget(node){}
    /**
    ...
    */
    IGraphConfig* iterateByType(const ia_uid &type) const;

private:
    IGraphConfig *iterateNodes(const ia_uid &type, const std::string &str) const;
    mutable int32_t mCurrentIndex; /**< state of the iterator */
