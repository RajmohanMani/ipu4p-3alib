...
typedef enum {
     ...
     * mode in HAL, all TuningModes used in AUTO mode must have a corresponding real ConfigMode
     * (except AUTO mode) which is configured in libcamhal_profile.xml.
     * If multiple TuningModes are supported in current pipe, TuningMode may be changed based scene.
     */
